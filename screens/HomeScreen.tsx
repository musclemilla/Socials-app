...
// Removed mock saved cards data

// Initialize SavedCards state with mock data
  const [SavedCards, setSavedCards] = useState<Profile[]>([]);
  const [influencers, setInfluencers] = useState<any[]>([]);
  const [isLoadingInfluencers, setIsLoadingInfluencers] = useState(true);

  // Fetch influencers with promo codes from Firebase
  useEffect(() => {
...
const [otherCards, setOtherCards] = useState([]);
  ...
  const [currentIndex, setCurrentIndex] = useState(0);  // Function to handle card transitions
  const handleNextCard = () => {
    if (!isAnimating) {
      setIsAnimating(true);
      
      // Calculate total available cards
      const totalCards = SavedCards.length > 0 
        ? Math.min(4, profiles.length + SavedCards.length) // Normal behavior with saved cards
        : 2; // Just user profile + invite card when no saved cards
    ...
    <View style={styles.inviteContainer}>
      ...
      <View style={styles.profileCardContainer}>       
         {profiles.length === 0 ? (
          ...
          (() => {
            // Create the card array based on saved cards availability
            const allCards = SavedCards.length > 0 
              ? [...profiles].concat(SavedCards.slice(0, 3)) // Normal: user profile + up to 3 saved cards
              : [...profiles, { // No saved cards: user profile + invite card
                  name: 'Invite Friends',
                  bio: 'No saved cards yet',
                  location: 'Invite contacts to connect',
                  image: { uri: 'https://api.a0.dev/assets/image?text=invitation%20card%20icon&aspect=1:1' },
                  bgColor: '#8e44ad',
                  borderColor: '#9b59b6',
                  chatApp: null,
                  isPublic: false,
            ...
            return allCards.slice(0, 4).map((profile, index) => {
              ...
              return (
                <Animated.View
                  key={`${index}-${profile.isInviteCard ? 'invite' : profile.name}`}
                  style={[
                    styles.profileCard,
                    {
                ...
                {index === 0 ? (
                  ...
                  <>
                    {profile.isInviteCard ? (                      /* Show Invite Contact button Profile Card */
                      <View style={styles.inviteCardContainer}>
                        <View style={styles.inviteCardGradient}>
                          <View style={styles.inviteCardHeader}>
                            <MaterialIcons name="group-add" size={34} color="#8e44ad" />
                            <Text style={styles.inviteCardTitle}>No Saved Cards</Text>
                          </View>

                          <View style={styles.inviteCardBody}>
                            <Image
                              source={{ 
                                uri: 'https://api.a0.dev/assets/image?text=group%20of%20people%20connecting%20and%20sharing%20digital%20cards%20modern%20illustration&aspect=1:1'
                              }}
...
//                                   await Share.share({
//                                     message: `🎯 Join me on Digital Cards! 
                                    
// Create and share your digital business card instantly. Connect with professionals worldwide and never lose a contact again.
...
// Download now and let's connect! 🚀`,
//                                     title: 'Join Digital Cards App',
//                                   });
...
//                               }}
                            >
                              ...
                              >
                                ...
                                <Text style={styles.inviteContactsButtonText}>Invite Contacts</Text>
                              </LinearGradient>
                            </TouchableOpacity>
                          </View>
                        </View>
                      ...
                      // New layout for subsequent cards (saved cards)
                      <View>
                        ...
                        <View style={styles.closedButtonContainer}>
                          ...
                          >
                            <Text style={styles.closedButtonText}>Saves</Text>
                          </TouchableOpacity>
                        </View>
                      </View>
                    )}
              ...
              <ScrollView style={styles.promosList}>
                {influencers.length > 0 ? (
                  ...
                  [...Array(1)].map((_, index) => (
                    <View key={index} style={styles.promoCard}>
                      <View style={styles.promoCardHeader}>
                        <View style={[styles.promoCardImage, { backgroundColor: '#f0f0f0' }]} />
                        <View style={styles.promoCardInfo}>
                          <View style={[styles.skeletonText, { width: 120, height: 20, backgroundColor: '#f0f0f0' }]} />
                          <View style={[styles.skeletonText, { width: 80, height: 16, backgroundColor: '#f0f0f0', marginTop: 4 }]} />
                        </View>
                        <View style={[styles.followButton, { backgroundColor: '#f0f0f0' }]} />
                      </View>
                      <View style={styles.promoCodes}>
                        <View style={styles.promoCodeContainer}>
                          <View style={styles.promoCodeInfo}>
                            <View style={[styles.skeletonText, { width: 100, height: 16, backgroundColor: '#f0f0f0' }]} />
                            <View style={[styles.skeletonText, { width: 80, height: 14, backgroundColor: '#f0f0f0', marginTop: 4 }]} />
                          </View>
                          <View style={[styles.copyCodeButton, { backgroundColor: '#f0f0f0' }]} />
                        </View>
                      </View>
                      <View style={[styles.codesEmptyBox]}>
