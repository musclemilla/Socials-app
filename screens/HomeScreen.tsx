...
const SavedCards = [
  ...
  const [showMoreCards, setShowMoreCards] = useState(false);
    const [otherCards, setOtherCards] = useState(SavedCards);
    ...
    setProfiles(currentProfiles => 
      currentProfiles.map((profile, idx) => 
        idx === currentIndex ? {
          ...profile,
          ...updatedProfile,
          chatApp: updatedProfile.chatApp,
  ...
  const pickImage = async () => {
    ...
    if (permissionResult.granted === false) {
      ...
      return;
        ...
        if (!result.canceled) {
          setProfileData(prev => ({ ...prev, image: result.assets[0].uri })); // Update profile image
        }
      };  
